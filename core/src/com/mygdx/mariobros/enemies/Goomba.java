...
public class Goomba extends Enemy {
    ...
    public Goomba(PlayScreen screen, float x, float y) {
        super(screen, x, y);//define trong nay luonn nroi
    ...
    public void update(float dt){
        ...
        if(setToDestroy&&!destroyed){
            ...
            setRegion(walkAnimation.getKeyFrame(stateTime, true));//ve len man hinh cac trang thai cua goomba
        }
    }
    @Override
    protected void defineEnemy() {
        ...
        fdef.shape=head;
        fdef.restitution=0.5f;//do dan hoi
        fdef.filter.categoryBits=MarioBros.HEAD_ENENY_BIT;
        b2body.createFixture(fdef).setUserData(this);

    }
    public void draw( Batch batch){
        if(!destroyed||stateTime<1){//xoa sau 1 giay con goomba bi de
            super.draw(batch);
        }
    }
    ...
    public void onEnemyHit(Enemy enemy) {
        if(enemy instanceof Turtle&&((Turtle)enemy).currentState== Turtle.State.MOVING_SHELL){
            setToDestroy=true;//huy trong ham update tuong tu nhu function hitOnHead
        }
